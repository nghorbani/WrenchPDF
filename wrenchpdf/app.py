...
from wrenchpdf.tempfiles import TempFileTracker, remove_temp_path
from wrenchpdf.utils import (
    InvalidImageError,
    PageAsset,
    assets_to_pdf_bytes,
    image_to_page_asset,
...
from wrenchpdf.version import version
...
def _handle_files(
    ...
    status = _format_status(
        f"Loaded {pages_count} page{'s' if pages_count != 1 else ''}. "
        "Drag rows on the left to reorder, click thumbnails to preview."
    )
    return (
